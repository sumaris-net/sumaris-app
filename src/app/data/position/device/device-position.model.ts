...
export class DevicePosition extends DataEntity<DevicePosition> implements IPositionEntity<DevicePosition> {
...
export class DevicePositionFilter extends EntityFilter<DevicePositionFilter, DevicePosition> {
  ...
  buildFilter(): FilterFn<DevicePosition>[] {
    ...
    if (this.objectId) {
      ...
      if (isNotNil(objectId)) filterFns.push((t) => t.objectId === objectId);
    }

    if (this.objectType) {
      const objectTypeLabel = this.objectType?.label;
      if (isNotNil(objectTypeLabel)) filterFns.push((t) => t.objectType.label === objectTypeLabel);
    }
