  ...
  implements OnInit, OnDestroy
{
    ...
    this.registerSubscription(
      this.$program
        .pipe(
          ...
          combineLatestWith(merge(this.$strategyLabel.pipe(distinctUntilChanged()), this._reloadStrategy$.pipe(map(() => this.$strategyLabel.value))))
        )
        .pipe(
  ...
  canUserWrite(data: T, opts?: any): boolean {
    return isNil(data.validationDate) && this.dataService.canUserWrite(data, { program: this.program, ...opts });
  }
  ...
  protected registerAutocompleteField<E = any, F = any>(
    fieldName: string,
    opts?: MatAutocompleteFieldAddOptions<E, F>
  ): MatAutocompleteFieldConfig<E, F> {
