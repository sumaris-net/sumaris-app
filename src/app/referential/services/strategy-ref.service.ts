...
export class StrategyRefService extends BaseReferentialService<
  Strategy, StrategyFilter, number,
  StrategyRefWatchOptions, StrategyRefLoadOptions,
  StrategyRefQueries> {
  ...
  watchByLabel(label: string,
               ...
               opts?: StrategyRefWatchOptions): Observable<Strategy> {
    if (!label) {
  ...
  async loadByFilter(filter?: Partial<StrategyFilter>, opts?: StrategyRefLoadOptions & {failIfMissing?: boolean; failIfMany?: boolean }): Promise<Strategy> {
  ...
  watchPmfms(
    ...
    opts?: StrategyRefWatchOptions): Observable<DenormalizedPmfmStrategy[]> {
