  ...
  static fromBatch(
    ...
    if (qvPmfm) {
      ...
      if (treeDepth < maxTreeDepth && isNotEmptyArray(pmfms)) {
        ...
        const childLabelPrefix = isCatchBatch ? `${AcquisitionLevelCodes.SORTING_BATCH}#` : `${samplingBatch?.label || batch.label}.`;
        const childrenPath = isCatchBatch ? 'children' : samplingBatch ? `${path}.children.0.children` : `${path}.children`;
        ...
        model.children = qvPmfm.qualitativeValues.map((qv, index) => {
          ...
          const childBatch =
            (samplingBatch?.children || batch.children || []).find((c) => PmfmValueUtils.equals(c.measurementValues?.[childQvPmfm.id], qv)) ||
            Batch.fromObject({
              measurementValues: {
          ...
          childModel.pmfms = [childQvPmfm, ...(childModel.pmfms || [])];
        ...
        model.childrenPmfms = [qvPmfm, ...pmfms];
      }
    } else {
      // No QV pmfm found
      model.pmfms = [];
