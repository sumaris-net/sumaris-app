...
const AggregatedLandingFragment = gql`
  fragment AggregatedLandingFragment on AggregatedLandingVO {
    __typename
    id
    observedLocationId
    vesselSnapshot {
...
const Mutations: BaseEntityGraphqlMutations = {
  saveAll: gql`
    mutation SaveAggregatedLandings($data: [AggregatedLandingVOInput], $filter: AggregatedLandingFilterVOInput) {
  ...
  deleteAll: gql`
    mutation DeleteAggregatedLandings($filter: AggregatedLandingFilterVOInput, $vesselIds: [Int]) {
  ...
  implements IEntitiesService<AggregatedLanding, AggregatedLandingFilter>
{
  ...
  async loadAllByObservedLocation(
    filter?: (AggregatedLandingFilter | any) & { observedLocationId: number },
    opts?: EntitiesServiceWatchOptions
  ): Promise<LoadResult<AggregatedLanding>> {
  ...
  watchAllByObservedLocation(
    filter?: (AggregatedLandingFilter | any) & { observedLocationId: number },
    opts?: EntitiesServiceWatchOptions
  ): Observable<LoadResult<AggregatedLanding>> {
  ...
  watchAllLocally(
    ...
    return this.entities.watchAll<AggregatedLanding>(AggregatedLanding.TYPENAME, variables, { fullLoad: opts && opts.fullLoad }).pipe(
      map(({ data, total }) => {
        const entities = !opts || opts.toEntity !== false ? (data || []).map(AggregatedLanding.fromObject) : ((data || []) as AggregatedLanding[]);
  ...
  async deleteAll(entities: AggregatedLanding[], options?: any): Promise<any> {
    ...
    const localIds = entities && entities.map((t) => t.id).filter((id) => id < 0);
    ...
    const ids = entities && entities.filter((entity) => entity.id === undefined && !!entity.vesselSnapshot.id);
