...
export class Sale extends DataRootVesselEntity<Sale, number, SaleAsObjectOptions, SaleFromObjectOptions> implements IWithProductsEntity<Sale> {
  ...
  asObject(opts?: SaleAsObjectOptions): any {
    ...
    target.fishingAreas = (this.fishingAreas && this.fishingAreas.filter(isNotNil).map((value) => value.asObject(opts))) || undefined;
    target.metiers = (this.metiers && this.metiers.filter(isNotNil).map((value) => value.asObject({ ...opts, ...NOT_MINIFY_OPTIONS }))) || undefined;
