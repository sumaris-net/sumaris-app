...
export class Sample
  extends RootDataEntity<Sample, number, SampleAsObjectOptions, SampleFromObjectOptions>
  implements IEntityWithMeasurement<Sample>, ITreeItemEntity<Sample>
{
        ...
        sources
          ...
          .reduce((res, source) => {
            ...
            if (isNotEmptyArray(source.children)) {
              return res.concat(target).concat(...this.treeAsObjectArray(source.children, { ...opts, parent: target }));
            }
  ...
  static equals(s1: Sample | any, s2: Sample | any): boolean {
    return (
      (s1 && s2 && isNotNil(s1.id) && s1.id === s2.id) ||
  ...
  asObject(opts?: SampleAsObjectOptions): any {
    ...
    target.taxonGroup =
      (this.taxonGroup &&
        this.taxonGroup.asObject({ ...opts, ...NOT_MINIFY_OPTIONS, keepEntityName: true /*fix #32*/ } as ReferentialAsObjectOptions)) ||
      undefined;
    target.taxonName =
      (this.taxonName &&
...
export class SampleUtils {
  ...
  static computeNextRankOrder(sources: Sample[], acquisitionLevel: AcquisitionLevelType) {
    return sources.filter((s) => this.hasAcquisitionLevel(s, acquisitionLevel)).reduce((max, s) => Math.max(max, s.rankOrder || 0), 0) + 1;
  }
  ...
  static filterIndividualMonitoring = (samples: Sample[]) =>
    SampleUtils.filterByAcquisitionLevel(samples, AcquisitionLevelCodes.INDIVIDUAL_MONITORING);
  ...
  static logSample(
    ...
    if (opts.showAll) {
      ...
      if (opts.showMeasure !== false && sample.measurementValues) {
        MeasurementValuesUtils.getPmfmIds(sample.measurementValues).forEach((pmfmId) => {
