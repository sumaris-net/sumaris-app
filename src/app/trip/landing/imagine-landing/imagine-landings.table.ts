...
import { AccountService, AppValidatorService, EntitiesTableDataSource, isNil, isNotNil } from '@sumaris-net/ngx-components';
import { ImagineLandingService } from './imagine-landing.service';
...
@Component({
  selector: 'app-imagine-landings-table',
  templateUrl: 'imagine-landings.table.html',
  styleUrls: ['imagine-landings.table.scss'],
  providers: [{ provide: AppValidatorService, useExisting: LandingValidatorService }],
  changeDetection: ChangeDetectionStrategy.OnPush,
})
export class ImagineLandingsTable extends BaseMeasurementsTable<ImagineLanding, ImagineLandingFilter> implements OnInit, OnDestroy {
  ...
  @Output() openTrip = new EventEmitter<TableElement<ImagineLanding>>();
  @Output() newTrip = new EventEmitter<TableElement<ImagineLanding>>();
  ...
  constructor(
    ...
    super(injector, ImagineLanding, ImagineLandingFilter, injector.get(ImagineLandingService), null, {
  ...
  setParent(parent: ObservedLocation | Trip | undefined) {
    if (isNil(parent?.id)) {
      ...
      this.setFilter(ImagineLandingFilter.fromObject({}));
      ...
      this.setFilter(ImagineLandingFilter.fromObject({ observedLocationId: parent.id }), { emitEvent: true /*refresh*/ });
    } else if (parent instanceof Trip) {
      this._parentDateTime = parent.departureDateTime;
      this.context.trip = parent;
      this.setFilter(ImagineLandingFilter.fromObject({ tripId: parent.id }), { emitEvent: true /*refresh*/ });
    }
  }
  ...
  getLandingDate(landing?: ImagineLanding): Moment {
  ...
  confirmAndEditTrip(event?: MouseEvent, row?: TableElement<ImagineLanding>): boolean {
  ...
  canCancelOrDelete(row: TableElement<ImagineLanding>): boolean {
    // IMAGINE-632: User can only delete landings or samples created by himself or on which he is defined as observer
    if (this.accountService.isAdmin()) {
      return true;
    }
