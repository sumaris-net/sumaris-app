...
import { BehaviorSubject, EMPTY, firstValueFrom, Observable, of, timer } from 'rxjs';
import { FetchPolicy, gql } from '@apollo/client/core';
import { DataCommonFragments, DataFragments } from '@app/trip/trip/trip.queries';
...
import { ImagineLanding } from '@app/trip/landing/imagine-landing/imagine-landing.model';
...
import { ImagineLandingFilter } from '@app/trip/landing/imagine-landing/imagine-landing.filter';
...
const sortByDateOrIdFn = (n1: ImagineLanding, n2: ImagineLanding) =>
  n1.dateTime.isSame(n2.dateTime) ? (n1.id === n2.id ? 0 : Math.abs(n1.id) > Math.abs(n2.id) ? 1 : -1) : n1.dateTime.isAfter(n2.dateTime) ? 1 : -1;

const sortByAscRankOrder = (n1: ImagineLanding, n2: ImagineLanding) => (n1.rankOrder === n2.rankOrder ? 0 : n1.rankOrder > n2.rankOrder ? 1 : -1);

const sortByDescRankOrder = (n1: ImagineLanding, n2: ImagineLanding) => (n1.rankOrder === n2.rankOrder ? 0 : n1.rankOrder > n2.rankOrder ? -1 : 1);

@Injectable({ providedIn: 'root' })
export class ImagineLandingService
  extends RootDataSynchroService<ImagineLanding, ImagineLandingFilter, number, LandingServiceLoadOptions>
  implements IEntitiesService<ImagineLanding, ImagineLandingFilter, LandingServiceWatchOptions>, IEntityService<ImagineLanding>
{
  ...
  constructor(
    ...
    super(injector, ImagineLanding, ImagineLandingFilter, {
  ...
  async loadAllByObservedLocation(
    filter?: (ImagineLandingFilter | any) & { observedLocationId: number },
    opts?: LandingServiceWatchOptions
  ): Promise<LoadResult<ImagineLanding>> {
  ...
  watchAllByObservedLocation(
    filter?: (ImagineLandingFilter | any) & { observedLocationId: number },
    opts?: LandingServiceWatchOptions
  ): Observable<LoadResult<ImagineLanding>> {
  ...
  watchAll(
    ...
    dataFilter?: Partial<ImagineLandingFilter>,
    opts?: LandingServiceWatchOptions
  ): Observable<LoadResult<ImagineLanding>> {
    ...
    return this.mutableWatchQuery<LoadResult<any>>({
      ...
      filter(() => !this.loading),
      map(({ data, total }) => {
        let entities = !opts || opts.toEntity !== false ? (data || []).map(ImagineLanding.fromObject) : ((data || []) as ImagineLanding[]);
        if (this._debug) {
          if (now) {
        ...
        if (isNotEmptyArray(entities) && groupByVessel) {
          const landingByVesselMap = new Map<number, ImagineLanding>();
        ...
        if (!opts || opts.computeRankOrder !== false) {
          this.computeRankOrderAndSort(entities, offset, total, afterSortBy, sortDirection, dataFilter as ImagineLandingFilter);
        }
      ...
      switchMap((res) =>
        timer(100)
          .pipe(map((_) => res))
          .pipe(
            mergeMap((_) => this.fillTaxonLocation(res.data).then(() => res)),
            startWith(res as LoadResult<any>)
          )
      )
    );
  }
  async fillTaxonLocation(entities: ImagineLanding[]): Promise<void> {
    const program: Strategy[] = [];
    const strategyLabels = new Set<string>(entities.map((x) => x['measurementValues'][PmfmIds.STRATEGY_LABEL]));
    for (const strategy of strategyLabels) {
      program.push(await this.strategyRefService.loadByLabel(strategy));
    }
    entities.forEach((entity) => {
      for (const strategy of program) {
        if (strategy.label == entity['measurementValues'][PmfmIds.STRATEGY_LABEL]) {
          entity.taxonNames = strategy.taxonNames;
          entity.appliedStrategies = strategy.appliedStrategies;
        }
      }
    });
  }
  async loadAll(
    ...
    sortBy?: string,
    sortDirection?: SortDirection,
    filter?: Partial<ImagineLandingFilter>,
    opts?: LandingServiceWatchOptions
  ): Promise<LoadResult<ImagineLanding>> {
  ...
  async loadAllLocally(
    ...
    const entities =
      !opts || opts.toEntity !== false ? (res.data || []).map((json) => this.fromObject(json)) : ((res.data || []) as ImagineLanding[]);

    return { data: entities, total: res.total };
  }

  async load(id: number, options?: EntityServiceLoadOptions): Promise<ImagineLanding> {
    ...
    try {
      ...
      // If local entity
      if (id < 0) {
        data = await this.entities.load<ImagineLanding>(id, ImagineLanding.TYPENAME);
      } else {
        // Load remotely
      ...
      // Transform to entity
      const entity = data && ImagineLanding.fromObject(data);
  ...
  async saveAll(entities: ImagineLanding[], opts?: LandingSaveOptions): Promise<ImagineLanding[]> {
  ...
  async saveAllLocally(entities: ImagineLanding[], opts?: LandingSaveOptions): Promise<ImagineLanding[]> {
    if (!entities) return entities;
    ...
    return chainPromises<ImagineLanding>(jobsFactories);
  }

  async save(entity: ImagineLanding, opts?: LandingSaveOptions): Promise<ImagineLanding> {
    const isNew = isNil(entity.id);
    ...
    const offlineResponse =
      !opts || opts.enableOptimisticResponse !== false
        ? async (context) => {
            ...
            if (isNotNil(entity.id)) context.serializationKey = `${ImagineLanding.TYPENAME}:${entity.id}`;
    ...
    await this.graphql.mutate<{ data: any }>({
      ...
      update: async (proxy, { data }) => {
        ...
        if (savedEntity.id < 0) {
          ...
          await this.entities.save<ImagineLanding>(savedEntity);
        }
   ...
   */
  async deleteLocally(filter: Partial<ImagineLandingFilter> & { observedLocationId: number }): Promise<ImagineLanding[]> {
    ...
    try {
      ...
      const res = await this.entities.loadAll<ImagineLanding>(ImagineLanding.TYPENAME, variables, { fullLoad: false });
      const ids = ((res && res.data) || []).map((o) => o.id);
      if (isEmptyArray(ids)) return undefined; // Skip

      // Apply deletion
      return await this.entities.deleteMany(ids, { entityName: ImagineLanding.TYPENAME });
    } catch (err) {
  ...
  watchAllLocally(
    ...
    sortBy?: string,
    sortDirection?: SortDirection,
    dataFilter?: ImagineLandingFilter | any,
    opts?: LandingServiceWatchOptions
  ): Observable<LoadResult<ImagineLanding>> {
    dataFilter = ImagineLandingFilter.fromObject(dataFilter);
    ...
    const entityName =
      !dataFilter.synchronizationStatus || dataFilter.synchronizationStatus !== 'SYNC'
        ? ImagineLanding.TYPENAME // Local entities
        : EntitiesStorage.REMOTE_PREFIX + ImagineLanding.TYPENAME; // Remote entities

    if (this._debug) console.debug(`[landing-service] Loading ${entityName} locally... using options:`, variables);
    return this.entities.watchAll<ImagineLanding>(entityName, variables, { fullLoad: opts?.fullLoad }).pipe(
      map(({ data, total }) => {
        const entities = !opts || opts.toEntity !== false ? (data || []).map(ImagineLanding.fromObject) : ((data || []) as ImagineLanding[]);
  ...
  async deleteAll(entities: ImagineLanding[], options?: any): Promise<any> {
    ...
    if (isNotEmptyArray(localIds)) {
      ...
      await this.entities.deleteMany<ImagineLanding>(localIds, { entityName: ImagineLanding.TYPENAME });
    }
  ...
  async synchronizeById(id: number): Promise<ImagineLanding> {
  ...
  async synchronize(entity: ImagineLanding, opts?: LandingSaveOptions): Promise<ImagineLanding> {
    opts = {
  ...
  async control(entity: ImagineLanding, opts?: LandingControlOptions): Promise<FormErrors> {
  ...
  async controlAllByObservedLocation(observedLocation: ObservedLocation, opts?: LandingControlOptions) {
    ...
    try {
      const { data } = await this.loadAllByObservedLocation(
        ImagineLandingFilter.fromObject({
  ...
  async executeImport(
    ...
    await this.entities.saveAll(data || [], {
      entityName: EntitiesStorage.REMOTE_PREFIX + ImagineLanding.TYPENAME,
      reset: true,
    });
  ...
  copyIdAndUpdateDate(source: ImagineLanding | undefined, target: ImagineLanding) {
    if (!source) return;
   ...
   * @param entity
   */
  async fixLandingTripDate(entity: ImagineLanding) {
   ...
   * @param opts
   */
  protected async saveLocally(entity: ImagineLanding, opts?: LandingSaveOptions): Promise<ImagineLanding> {
  ...
  protected asObject(source: ImagineLanding, opts?: DataEntityAsObjectOptions): any {
  ...
  protected fillDefaultProperties(entity: ImagineLanding, opts?: Partial<LandingSaveOptions>) {
  ...
  protected async fillOfflineDefaultProperties(entity: ImagineLanding) {
    const isNew = isNil(entity.id);
  ...
  protected async fillControlOptions(entity: ImagineLanding, opts?: LandingControlOptions): Promise<LandingControlOptions> {
   ...
   * @param targets
   */
  protected copyIdAndUpdateDateOnSamples(savedLanding: ImagineLanding, sources: (Sample | any)[], targets: Sample[]) {
    // Update samples
    if (sources && targets) {
  ...
  copyIdAndUpdateDateOnTrip(savedLanding: ImagineLanding, source: Trip | undefined, target: Trip) {
