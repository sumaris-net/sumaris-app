...
export class PhysicalGearsTestPage implements OnInit {
  ...
  ngOnInit() {
    ...
    // Modal options
    this.filterForm.get('modalOptions').valueChanges.subscribe((value) => this.applyModalOptions(value));
  ...
  applyModalOptions(modalOptions?: any) {
    ...
    Object.keys(modalOptions).forEach((key) => this.setModalOptions(key as keyof IPhysicalGearModalOptions, modalOptions[key]));
  }
