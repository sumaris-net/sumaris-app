  ...
  implements IEntityWithMeasurement<PhysicalGear>, ITreeItemEntity<PhysicalGear>
{
  ...
  static computeSameAsScore(
    reference: PhysicalGear,
    source?: PhysicalGear,
    opts?: { withMeasurementValues?: boolean; withRankOrder?: boolean; tripId?: boolean }
  ): number {
    if (!source) return -1;

    return (
      (reference.gear?.id === source.gear?.id ? 1 : 0) * 1000 +
  ...
  static scoreComparator(
    ...
    opts?: { withMeasurementValues?: boolean; withRankOrder?: boolean; tripId?: boolean }
  ): (g1: PhysicalGear, g2: PhysicalGear) => number {
