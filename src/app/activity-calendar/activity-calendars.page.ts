...
import { ActivityCalendarTrashModal, ActivityCalendarTrashModalOptions } from './trash/activity-calendar-trash.modal';
import {
...
import { ActivityCalendarOfflineModal, ActivityCalendarOfflineModalOptions } from '@app/activity-calendar/offline/activity-calendar-offline.modal';
import { DataQualityStatusEnum, DataQualityStatusList } from '@app/data/services/model/model.utils';
import { ContextService } from '@app/shared/context.service';
import { ActivityCalendarContextService } from '@app/activity-calendar/activity-calendar-context.service';
...
export class ActivityCalendarsPage extends AppRootDataTable<ActivityCalendar, ActivityCalendarFilter> implements OnInit, OnDestroy {
  ...
  constructor(
    ...
    super(injector,
      ActivityCalendar, ActivityCalendarFilter,
      ['quality',
      'program',
      'vessel',
      'departureLocation',
    ...
    this.registerSubscription(
      this.configService.config
        .pipe(
          filter(isNotNil),
          tap(config => {
            console.info('[activityCalendars] Init from config', config);

            const title = config && config.getProperty(ACTIVITY_CALENDAR_CONFIG_OPTIONS.ACTIVITY_CALENDAR_NAME);
  ...
  onSwipeTab(event: HammerSwipeEvent): boolean {
    // DEBUG
    // if (this.debug) console.debug("[activityCalendars] onSwipeTab()");
  ...
  async openTrashModal(event?: Event) {
    console.debug('[activityCalendars] Opening trash modal...');
    const modalOptions: ActivityCalendarTrashModalOptions = {
      synchronizationStatus: this.filter && this.filter.synchronizationStatus || 'SYNC'
    };
    const modal = await this.modalCtrl.create({
      component: ActivityCalendarTrashModal,
    ...
    // Select the local activityCalendars tab
    this.setSynchronizationStatus('DIRTY');

    // If only one restored: open it
    const activityCalendar = data?.length === 1 && data[0];
    if (isNotNil(activityCalendar.id)) {
      await this.navController.navigateForward([activityCalendar.id], {
  ...
  async prepareOfflineMode(event?: Event, opts?: {
    ...
    const feature = this.settings.getOfflineFeature(this._dataService.featureName) || <OfflineFeature>{
  ...
  async importFromFile(event: Event): Promise<ActivityCalendar[]> {
    ...
    if (isEmptyArray(entities) && isEmptyArray(errors)) {
      ...
      await this.showToast({
        type: 'error',
        message: 'ACTIVITY_CALENDAR.TABLE.ERROR.IMPORT_FILE_FAILED', messageParams: {error: errors.join('\n')}});
    }
    else if (isNotEmptyArray(errors)) {
      await this.showToast({
        type: 'warning',
        message: 'ACTIVITY_CALENDAR.TABLE.INFO.IMPORT_FILE_SUCCEED_WITH_ERRORS', messageParams: {inserts: entities.length, errors: errors.length}});
    }
    else {
      await this.showToast({
        type: 'info',
        message: 'ACTIVITY_CALENDAR.TABLE.INFO.IMPORT_FILE_SUCCEED', messageParams: {inserts: entities.length}});
    }
    return entities;
  }
  ...
  async openDownloadPage(type?: ExtractionType) {
    const activityCalendars = (this.selection.selected || [])
      .map(row => row.currentData).filter(isNotNil);
    if (isEmptyArray(activityCalendars)) return; // Skip if empty

    const programs = arrayDistinct(activityCalendars.map(t => t.program), 'label');
  ...
  protected resetContext() {
    ...
    this.context.reset();
    this.activityCalendarContext.reset();
  }
