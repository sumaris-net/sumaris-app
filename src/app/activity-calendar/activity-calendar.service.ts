...
const ActivityCalendarQueries: BaseEntityGraphqlQueries = {
  ...
  loadAll: gql` query ActivityCalendars($offset: Int, $size: Int, $sortBy: String, $sortDirection: String, $trash: Boolean, $filter: ActivityCalendarFilterVOInput){
  ...
  loadAllWithTotal: gql` query ActivityCalendars($offset: Int, $size: Int, $sortBy: String, $sortDirection: String, $trash: Boolean, $filter: ActivityCalendarFilterVOInput){
    data: activityCalendars(filter: $filter, offset: $offset, size: $size, sortBy: $sortBy, sortDirection: $sortDirection, trash: $trash){
      ...LightActivityCalendarFragment
    }
...
const ActivityCalendarSubscriptions = {
  listenChanges: gql`subscription UpdateActivityCalendar($id: Int!, $interval: Int){
...
export class ActivityCalendarService
  ...
  watchAll(offset: number,
           ...
           opts?: ActivityCalendarWatchOptions): Observable<LoadResult<ActivityCalendar>> {
  ...
  watchAllLocally(offset: number,
                  ...
                  sortDirection?: SortDirection,
                  dataFilter?: Partial<ActivityCalendarFilter>,
                  options?: ActivityCalendarWatchOptions): Observable<LoadResult<ActivityCalendar>> {
    ...
    return this.entities.watchAll<ActivityCalendar>(ActivityCalendar.TYPENAME, variables)
      .pipe(
        map(res => {
  ...
  listenChanges(id: number, opts?: EntityServiceListenChangesOptions): Observable<ActivityCalendar> {
    ...
    if (EntityUtils.isLocalId(id)) {
      ...
      return this.entities.watchAll<ActivityCalendar>(ActivityCalendar.TYPENAME, {offset:0, size: 1, filter: (t) => t.id === id})
        .pipe(
          map(({data}) => {
  ...
  async control(entity: ActivityCalendar, opts?: ActivityCalendarControlOptions): Promise<AppErrorWithDetails> {
    ...
    if (!form.valid) {
      ...
      if (form.invalid) {
        ...
        if (this._debug) console.debug(`[activity-calendar-service] Control activityCalendar {${entity.id}} [INVALID] in ${Date.now() - now}ms`, errors);
        return {
   ...
   * @param opts
   */
  async deleteAll(entities: ActivityCalendar[], opts?: {
   ...
   * @param opts
   */
  async deleteAllLocally(entities: ActivityCalendar[], opts?: {
    trash?: boolean; // True by default
  }): Promise<any> {
  ...
  async deleteLocally(entity: ActivityCalendar, opts?: {
    trash?: boolean; // True by default
  }): Promise<any> {
   ...
   * @param opts
   */
  protected getImportJobs(filter: Partial<ActivityCalendarFilter>, opts: {
   ...
   * @protected
   */
  protected async importHistoricalData(filter?: Partial<ActivityCalendarFilter>,
                                       opts?: {
