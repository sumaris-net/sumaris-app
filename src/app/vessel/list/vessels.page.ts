...
export class VesselsPage implements OnInit, OnDestroy {
  ...
  get replacementDisabled(): boolean {
    return (
      (!this.canReplace && this.table.selection.isEmpty()) ||
  ...
  async replace(event: MouseEvent) {
    ...
    if (data && data[0] instanceof VesselSnapshot) {
      ...
      if (
        await Alerts.askConfirmation('VESSEL.ACTION.REPLACE_MANY_CONFIRMATION', this.alertCtrl, this.translate, event, {
          vessel: referentialToString(vessel, ['registrationCode', 'name']),
        })
      ) {
        try {
          // Replace temp vessels (from selected rows)
          await this.vesselService.replaceTemporaryVessel(
            this.table.selection.selected.map((row) => row.currentData.id),
            vessel.id
          );
        } catch (e) {
  ...
  async importFromCsvFile(event?: UIEvent, format = 'siop') {
    ...
    if (isNotEmptyArray(uploadedFileNames)) {
      const jobs = await Promise.all(uploadedFileNames.map((uploadedFileName) => this.vesselService.importFile(uploadedFileName, format)));
