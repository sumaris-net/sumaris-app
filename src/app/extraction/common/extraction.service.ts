...
const Queries = {
  ...
  `,
  techMinMax: gql`
    query AggregationTechMinMax($type: ExtractionTypeVOInput!, $filter: ExtractionFilterVOInput, $strata: AggregationStrataVOInput) {
...
export class ExtractionService extends BaseGraphqlService<ExtractionType, ExtractionTypeFilter> {
  ...
  asFilter(source?: Partial<ExtractionFilter> | any): ExtractionFilter {
    ...
    target.criteria = (target.criteria || []).filter((criterion) => isNotNil(criterion.name)).filter(ExtractionFilterCriterion.isNotEmpty);
  ...
  ): Promise<{ aggMin: number; aggMax: number; techMin: number; techMax: number }> {
    filter = this.asFilter(filter);
