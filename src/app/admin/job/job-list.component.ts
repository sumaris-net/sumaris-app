...
export class JobListComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    this.state.connect('jobs',
      merge(
        ...
        this.onRefresh.pipe(
          map(_ => ({issuer: this.issuer, status: this.status, types: this.types}))
        )
      )
      .pipe(
    ...
    this.state.connect('availableTypes', this.jobService.watchTypes().pipe(
      map(availableTypes => {
    ...
    this._subscription.add(
      this.translate.get(statusNames)
  ...
  getFilter({issuer, types, status}: {issuer: string; types: JobTypeLabel[]; status: JobStatusLabel[]}) {
    ...
    // Issuer
    let filterIssuer = this.filterForm.get('issuer').value as Person;
    issuer = EntityUtils.isNotEmpty(filterIssuer, 'id') ? (filterIssuer?.pubkey || 'NO_PUBKEY') : issuer;

    // Type
    let filterType = this.filterForm.get('types').value as IJobType[];
    types = isNotEmptyArray(filterType) ? filterType.map(t => t.label) : types;

    // Read filter's status
    let filterStatus = this.filterForm.get('status').value as IJobStatus[];
