...
export class AuctionControlValidators {
    static addSampleValidators(form, pmfms, opts) {
        if (!form) {
        ...
        AppFormUtils.disableControls(form, pmfms.filter((p) => p.isComputed).map((p) => `measurementValues.${p.id}`), { onlySelf: true, emitEvent: false });
        const $errors = new Subject();
        form.setAsyncValidators((control) => $errors);
        let computing = false;
        const subscription = form.valueChanges
            .pipe(startWith(form.value), filter(() => !computing), 
        // Protected against loop
        tap(() => (computing = true)), debounceTime(250), map(() => AuctionControlValidators.computeAndValidate(form, pmfms, Object.assign(Object.assign({}, opts), { emitEvent: false, onlySelf: false }))), tap((errors) => {
            computing = false;
            $errors.next(errors);
     ...
     * @param opts
     */
    static computeAndValidate(form, pmfms, opts) {
        var _a, _b;
        ...
        if (compliantProductControl) {
            ...
            if (controlCorrectiveActionControl) {
                const defaultValue = PmfmValueUtils.fromModelValue(controlCorrectiveActionPmfm.defaultValue, controlCorrectiveActionPmfm) ||
                    ((_a = controlCorrectiveActionPmfm.qualitativeValues) === null || _a === void 0 ? void 0 : _a.find((qv) => qv.label === 'NSP'));
            ...
            if (productDestinationControl) {
                const defaultValue = PmfmValueUtils.fromModelValue(productDestinationPmfm.defaultValue, productDestinationPmfm) ||
                    ((_b = productDestinationPmfm.qualitativeValues) === null || _b === void 0 ? void 0 : _b.find((qv) => qv.label === 'NSP'));
                if (compliantProductControl.value) {
                    ...
                    if (compliantProductControl.value === false && isNil(productDestinationControl.value))
                        productDestinationControl.setValue(defaultValue);
...
//# sourceMappingURL=auction-control.validators.js.map