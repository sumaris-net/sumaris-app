...
import { AggregatedLanding, AggregatedLandingUtils } from './aggregated-landing.model';
import { BaseGraphqlService, chainPromises, DateUtils, EntitiesStorage, EntityUtils, firstNotNilPromise, GraphqlService, isEmptyArray, isNil, isNotEmptyArray, isNotNil, NetworkService, toNumber, } from '@sumaris-net/ngx-components';
...
let AggregatedLandingService = class AggregatedLandingService extends BaseGraphqlService {
    constructor(injector, network, tripService, entities) {
    ...
    watchAll(offset, size, sortBy, sortDirection, dataFilter, opts) {
        ...
        if (this._debug)
            console.debug('[aggregated-landing-service] Loading aggregated landings... using options:', variables);
    ...
    watchAllLocally(offset, size, sortBy, sortDirection, dataFilter, opts) {
        ...
        if (!EntityUtils.isLocalId(dataFilter.observedLocationId))
        ...
        if (this._debug)
            console.debug(`[aggregated-landing-service] Loading aggregated locally... using options:`, variables);
        return this.entities.watchAll(AggregatedLanding.TYPENAME, variables, { fullLoad: opts && opts.fullLoad })
            .pipe(map(({ data, total }) => {
    ...
    saveAll(entities, options) {
        return __awaiter(this, void 0, void 0, function* () {
            if (!entities)
                return entities;
            const filter = this.asFilter((options === null || options === void 0 ? void 0 : options.filter) || this._lastFilter);
            ...
            const now = Date.now();
            if (this._debug)
                console.debug('[aggregated-landing-service] Saving aggregated landings...', json);
            yield this.graphql.mutate({
                mutation: Mutations.saveAll,
                variables: {
                    data: json,
                    filter: filter === null || filter === void 0 ? void 0 : filter.asPodObject()
                },
                context: {
                    tracked: false
                },
                ...
                update: (cache, { data }) => {
                    ...
                    if (this._debug)
                        console.debug(`[aggregated-landing-service] Aggregated landings saved remotely in ${Date.now() - now}ms`, savedEntities);
                    const newEntities = [];
                    // Update ids
    ...
    saveAllLocally(entities, opts) {
        return __awaiter(this, void 0, void 0, function* () {
            if (!entities)
                return entities;
            if (this._debug)
                console.debug(`[aggregated-landing-service] Saving ${entities.length} aggregated landings locally...`);
    ...
    deleteAll(entities, options) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (isNotEmptyArray(localIds)) {
                if (this._debug)
                    console.debug('[aggregated-landing-service] Deleting aggregated landings locally... ids:', localIds);
                yield this.entities.deleteMany(localIds, { entityName: AggregatedLanding.TYPENAME });
            }
            ...
            const now = Date.now();
            if (this._debug)
                console.debug('[aggregated-landing-service] Deleting aggregated landings... ids:', ids);
            yield this.graphql.mutate({
                ...
                update: (proxy) => {
                    ...
                    if (this._debug)
                        console.debug(`[aggregated-landing-service] Aggregated Landings deleted in ${Date.now() - now}ms`);
                }
            });
        });
    }
    synchronizeAll(entities, opts) {
        return __awaiter(this, void 0, void 0, function* () {
            const filter = this.asFilter(opts === null || opts === void 0 ? void 0 : opts.filter);
            if (!filter || filter.isEmpty())
                throw new Error('Missing options filter arguments');
            ...
            let target = entities.map(source => {
                ...
                copy.observedLocationId = (_a = opts === null || opts === void 0 ? void 0 : opts.filter) === null || _a === void 0 ? void 0 : _a.observedLocationId;
                copy.id = undefined;
                // target.vesselActivities?.forEach(activity => activity.tripId = undefined);
                return copy;
            });
            target = yield this.saveAll(target, { filter });
            yield this.deleteAll(entities);
            return target;
        });
    }
    asFilter(filter) {
    ...
    asObject(entity, options) {
        options = Object.assign(Object.assign({}, MINIFY_OPTIONS), options);
        const copy = entity.asObject(options);
    ...
    saveLocally(entity, opts) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (!EntityUtils.isLocalId(entity.observedLocationId))
            ...
            if (this._debug)
                console.debug('[aggregated-landing-service] [offline] Saving aggregated landing locally...', json);
    ...
    fillOfflineDefaultProperties(entity, opts) {
        ...
        return __awaiter(this, void 0, void 0, function* () {
            ...
            entity.observedLocationId = toNumber(entity.observedLocationId, (_a = opts === null || opts === void 0 ? void 0 : opts.filter) === null || _a === void 0 ? void 0 : _a.observedLocationId);
            // Fill default synchronization status
            entity.synchronizationStatus = entity.synchronizationStatus || 'DIRTY';
        });
    }
};
AggregatedLandingService = __decorate([
...
export { AggregatedLandingService };
//# sourceMappingURL=aggregated-landing.service.js.map