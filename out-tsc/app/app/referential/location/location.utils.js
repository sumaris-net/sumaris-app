...
export class LocationUtils {
     ...
     */
    static getRectangleLabelByLatLong(lat, lon) {
        if (isNil(lat) || isNil(lon))
            return undefined; // Skip
        let result;
        // If position inside "Mediterranean and black sea" :
        if ((lon >= 0 && lon < 42 && lat >= 30 && lat < 47.5) || (lon >= -6 && lon < 0 && lat >= 35 && lat < 40)) {
    ...
    static isStatisticalRectangleLabel(label) {
        return (label && LocationUtils.RECTANGLE_LABEL_REGEXP.test(label)) || false;
    }
    static getGeometryFromRectangleLabel(rectangleLabel, opts = { checkValid: true }) {
        const bbox = LocationUtils.getBBoxFromRectangleLabel(rectangleLabel);
        // Check is valid
        if (opts.checkValid && !bbox)
            throw new Error(`Invalid statistical rectangle '${rectangleLabel}': unknown format`);
        return (bbox && Geometries.createRectangleGeometry(bbox[0], bbox[1], bbox[2], bbox[3], false)) || undefined;
    }
    static getBBoxFromRectangleLabel(rectangleLabel) {
        if (!rectangleLabel || !this.isStatisticalRectangleLabel(rectangleLabel))
...
LocationUtils.RECTANGLE_LABEL_REGEXP = /^M?[0-9]{2,3}[A-Z][0-9]{1,2}$/;
