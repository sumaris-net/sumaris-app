...
import { AccountService, BaseGraphqlService, CryptoService, EntityUtils, escapeRegExp, firstNotNilPromise, GraphqlService, isEmptyArray, isNil, isNilOrBlank, isNotNil, ReferentialUtils, StatusIds, } from '@sumaris-net/ngx-components';
...
let ProductService = class ProductService extends BaseGraphqlService {
    constructor(graphql, extractionTypeService, accountService, translateService) {
     ...
     * Watch products
     */
    watchAll(dataFilter, options) {
        if (this._debug)
            console.debug('[product-service] Loading products...');
        ...
        return this.mutableWatchQuery({
            ...
            .pipe(map((data) => {
            const entities = ((data === null || data === void 0 ? void 0 : data.data) || []).map(ExtractionProduct.fromObject);
            return {
                data: entities,
        ...
        return __awaiter(this, void 0, void 0, function* () {
            ...
            while (!unique) {
                ...
                if (types) {
                    ...
                    unique = !(yield this.extractionTypeService.existsByLabel(label, { fetchPolicy: 'no-cache' }));
                }
                if (unique)
                    return label;
            }
    ...
    computeNextName(name, types) {
        var _a;
        return __awaiter(this, void 0, void 0, function* () {
            if (isNilOrBlank(name))
                return name; // Skip if blank
            // Fetch all types
            types = types || ((_a = (yield firstNotNilPromise(this.watchAll({
                searchAttribute: 'name', searchText: name
            })))) === null || _a === void 0 ? void 0 : _a.data);
            if (isEmptyArray(types))
                return name; // Skip if no other types
            name = name.trim();
     ...
     * @param options
     */
    loadColumns(type, sheetName, options) {
        return __awaiter(this, void 0, void 0, function* () {
            const variables = {
            ...
            const now = Date.now();
            if (this._debug)
                console.debug('[product-service] Loading columns... using options:', variables);
            ...
            if (this._debug)
                console.debug(`[product-service] Columns ${type.category} ${type.label} loaded in ${Date.now() - now}ms`, data);
            return data;
        });
    }
    ...
    save(entity, filter) {
        return __awaiter(this, void 0, void 0, function* () {
            const now = Date.now();
            if (this._debug)
                console.debug('[product-service] Saving product...');
            ...
            if (this._debug)
                console.debug('[product-service] Using minify object, to send:', json);
    ...
    delete(type) {
        return __awaiter(this, void 0, void 0, function* () {
            if (!type || isNil(type.id))
                throw Error('Missing type or type.id');
            const now = Date.now();
            if (this._debug)
                console.debug(`[product-service] Deleting product {id: ${type.id}'}`);
            yield this.graphql.mutate({
                ...
                update: (cache) => {
                    ...
                    cache.evict({ id: cache.identify(Object.assign(Object.assign({}, cacheKey), { __typename: ExtractionType.TYPENAME })) });
                    if (this._debug)
                        console.debug(`[product-service] Product deleted in ${Date.now() - now}ms`);
                }
            });
        });
    }
    ...
    updateProduct(id) {
        return __awaiter(this, void 0, void 0, function* () {
            const now = Date.now();
            if (this._debug)
                console.debug(`[product-service] Updating extraction product #{id}...`);
            let savedEntity;
            yield this.graphql.mutate({
